  ...
  Widget build(BuildContext context) {
    return BlocProvider.value(
      ...
      child: Scaffold(
        ...
        appBar: WGradientAppBar(title: 'quran_downloads_title'.tr()),
        body: BlocBuilder<CBReciterDownloads, SReciterDownloads>(
          builder: (context, state) {
            if (state.status == LoadStatus.loading && state.reciters.isEmpty) {
              return const Center(child: CircularProgressIndicator());
            }
            if (state.status == LoadStatus.error && state.reciters.isEmpty) {
              return Center(child: Text(state.error ?? 'common_error'.tr()));
            }
