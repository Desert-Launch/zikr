...
/// Haramain live broadcasts. A single embedded [WebView] fills the screen and
/// plays whichever official channel is selected; the toggle reloads it. Streams
/// play a specific live video (see [ELiveChannel]) — `controls=0` keeps the
/// surface immersive and the app chrome (header + switcher) overlays on top.
///
...
class SNLive extends StatefulWidget {
  const SNLive({super.key});

  @override
  State<SNLive> createState() => _SNLiveState();
}

class _SNLiveState extends State<SNLive> {
  ...
  ELiveChannel _channel = ELiveChannel.makkah;
  bool _loading = true;
  bool _error = false;
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onWebResourceError: (error) {
            ...
            if (!_usedFallback) {
              _usedFallback = true;
              _controller.loadRequest(Uri.parse(_channel.fallbackUrl));
            } else {
              setState(() {
                _loading = false;
                _error = true;
...
<body>
  ...
  void _load(ELiveChannel channel) {
    _usedFallback = false;
    setState(() {
      _loading = true;
      _error = false;
    });
    _controller.loadHtmlString(_embedHtml(channel.embedUrl), baseUrl: _origin);
  }

  void _select(ELiveChannel channel) {
  ...
  Future<void> _openInYoutube() async {
    final uri = Uri.parse(_channel.fallbackUrl);
    ...
    return WSharedScaffold(
      ...
      body: Stack(
        ...
        children: [
          ...
          ColoredBox(
            color: Colors.black,
            child: Center(child: _error ? _errorView() : _video()),
          ),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              _PillButton(label: 'live_retry'.tr(), filled: true, onTap: () => _load(_channel)),
